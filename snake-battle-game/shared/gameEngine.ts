...
export const MAX_TURN_RATE = Math.PI * 3.2; // rad/sec (≈576°/s) — 更跟手的转向上限（Slither 风格）
...
export function setSnakeStick(state: GameState, snakeId: string, stick: Vec2) {
  ...
  const DEADZONE = 0.06; // 更小的死区，提升摇杆跟手性
  if (m < DEADZONE) {
...
export function updateGame(state: GameState, dtMs: number) {
  ...
  for (const s of state.snakes) {
    ...
    // 低幅度输入也能有可感知的转向，但满幅度时更快
    const maxDelta = MAX_TURN_RATE * dt * (0.25 + 0.75 * strength);
