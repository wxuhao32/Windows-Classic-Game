...
function getWsUrl() {
  ...
  const room = (search.get('room') || 'public').trim();
  ...
  try {
    ...
    // base 可能不是完整 URL（例如 ws://... 已是完整，但保险兜底）
...
export default function Game() {
  ...
  const roomInfo = useMemo(() => {
    const params = new URLSearchParams(window.location.search);
    return {
      room: (params.get('room') || 'public').trim() || 'public',
  ...
  const copyInvite = useCallback(() => {
    ...
    // 直接给到 /game 链接：打开即入房间
    u.pathname = "/game";
    u.searchParams.set("mode", "online");
    u.searchParams.set("room", roomInfo.room || "public");
    if (roomInfo.key) u.searchParams.set("key", roomInfo.key); else u.searchParams.delete("key");
    if (roomInfo.name) u.searchParams.set("name", roomInfo.name); else u.searchParams.delete("name");
    navigator.clipboard?.writeText?.(u.toString());
    toast.success("已复制邀请链接（发给朋友即可进入同房间）");
  }, [roomInfo]);

  const [gameState, setGameState] = useState<GameState>(() =>
    mode === 'online'
      ? initializeArena(GAME_WIDTH, GAME_HEIGHT, 12)
  ...
  const toggleFullscreen = useCallback(async () => {
    try {
      if (!document.fullscreenElement) {
        ...
        // @ts-expect-error - Safari/older browsers may not support options
        await el.requestFullscreen?.({ navigationUI: 'hide' });
        try {
          // 仅部分浏览器支持，且通常要求在全屏后调用
      ...
      toast.error('当前浏览器不支持全屏或被阻止（请用系统全屏手势/按钮）');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === 'welcome') {
        ...
        // welcome 时可能还没接管
        const mine = msg.state.snakes.find((s) => s.controlledBy === msg.clientId);
  ...
  const availableSnakes = useMemo(() => {
    if (mode !== 'online') return [];
    return gameState.snakes.filter((s) => s.isAlive && !s.isPlayer);
  }, [gameState.snakes, mode]);
          ...
          <div className="bg-[#1a1f2e] border-2 border-[#00ffff] rounded-lg p-4 mb-6">
            ...
            <div className="mt-4">
              <div className="text-[#a0a0a0] text-xs uppercase tracking-wider mb-2">
                {mySnakeId ? '切换接管到另一条 AI 蛇' : '选择一个 AI 蛇接管开始游戏'}
              </div>
              <div className="flex flex-wrap gap-2">
                {availableSnakes.length === 0 ? (
                  <div className="text-[#ff6600] text-sm">暂无可接管的 AI（可能都已被占用或已死亡）</div>
                ) : (
                  availableSnakes.map((s) => (
                    <button
                      key={s.id}
